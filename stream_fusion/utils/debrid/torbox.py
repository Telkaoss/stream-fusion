    ...
    def _select_file(self, torrent_info, stream_type, file_index, season, episode):
        ...
        if stream_type == "movie":
            ...
            if matching_files:
                ...
                if fallback_file:
                    logger.info(f"Torbox: Smart fallback a sélectionné: {fallback_file.get('name')} (ID: {fallback_file.get('index')})")
                    return fallback_file.get('index')
                else:
                    logger.info("Torbox: Smart fallback n'a rien trouvé, essai du fallback final pour fichier unique")
                    ...
                    logger.info(f"Torbox: DEBUG - Trouvé {len(video_files)} fichiers vidéo dans le fallback final")
                    for vf in video_files:
                        logger.info(f"Torbox: DEBUG - Fichier vidéo: {vf['short_name']} (ID: {vf['id']})")
